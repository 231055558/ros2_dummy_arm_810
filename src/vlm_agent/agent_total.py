...
import rclpy
from simple_moveit_controller import SimpleMoveItController
...
PREDEFINED_POSES = {
    ...
    "presentation_pose": [0, 60, -30, 0, -60, 0]
}
SWAP_CALIBRATION_X_POINTS = False
SWAP_CALIBRATION_Y_POINTS = False

# ---------------------------------------------------------------------------
# 2. 辅助类与函数 (CameraCalibrator) (无修改)
# ---------------------------------------------------------------------------
class CameraCalibrator:
    def __init__(self, **config):
        self.img_w = config["image_width"]; self.img_h = config["image_height"]
        px1, py1 = config["point1_px"]; wx1, wy1 = config["point1_world"]
        px2, py2 = config["point2_px"]; wx2, wy2 = config["point2_world"]
        if SWAP_CALIBRATION_X_POINTS: wx1, wx2 = wx2, wx1
        if SWAP_CALIBRATION_Y_POINTS: wy1, wy2 = wy2, wy1
...
# ---------------------------------------------------------------------------
# 3. 机器人控制器适配器 (RobotController) (无修改)
# ---------------------------------------------------------------------------
class RobotController:
    ...
    def move_to_location(self, location_name: str, memorized_locations: Dict[str, List[float]]) -> bool:
        ...
        if location_name in PREDEFINED_POSES:
            ...
            x, y, z = xyz; rx, ry, rz = TARGET_ORIENTATION_RPY
            ...
            print(f"!!! [Robot] 错误: 未知的目标位置 '{location_name}'"); return False
    def capture_and_save_image(self) -> str:
        print("[Robot] 正在通过 RealSense 拍照..."); image_path = "latest_capture.jpg"; pipeline = None
        try:
            ...
            frames = pipeline.wait_for_frames(); color_frame = frames.get_color_frame()
            if not color_frame: raise RuntimeError("无法从 RealSense 获取彩色图像帧。")
            color_image = np.asanyarray(color_frame.get_data()); cv2.imwrite(image_path, color_image)
            print(f"[Robot] 图片已成功保存至 '{image_path}'。"); return image_path
        except Exception as e:
            print(f"!!! [Robot] 拍照失败: {e}"); return ""
        finally:
            if pipeline: pipeline.stop()
    def perform_pick(self):
        print("[Robot] 正在执行抓取序列..."); current_pose = self.controller.get_current_ee_pose()
        if not current_pose: print("!!! [Robot] 无法获取当前位姿，抓取中止。"); return False
        x, y, z, rx, ry, rz = current_pose
        print(f"[Robot] 从 Z={z:.3f} 向下移动 {PICK_DEPTH} 米...")
        if not self.controller.move_to_cartesian_pose(x, y, z - PICK_DEPTH, rx, ry, rz):
            print("!!! [Robot] 向下移动失败，抓取中止。"); return False
        print("[Robot] 使能夹爪...")
        if not self.controller.set_gripper_enable(True):
            print("!!! [Robot] 夹爪使能失败。"); return False
        print("[Robot] 闭合夹爪...")
        if not self.controller.control_gripper(close_gripper=True):
             print("!!! [Robot] 闭合夹爪失败。"); return False
        sleep(1.0); print("[Robot] 抓取完成。"); return True
    def perform_drop(self):
        ...
        sleep(1.0); return success
    def shutdown(self):
        print("[Robot] 正在安全关闭机器人..."); self.controller.enable_robot(False); self.controller.destroy_node()

# ---------------------------------------------------------------------------
# 4. 【核心】任务编排Agent (TaskOrchestrator) (已修改)
# ---------------------------------------------------------------------------
class TaskOrchestrator:
    def __init__(self, ros_controller_node: SimpleMoveItController):
        ...
        self.vlm = VLMDetector(
            image_width=CALIBRATION_CONFIG["image_width"],
            image_height=CALIBRATION_CONFIG["image_height"]
        )
    ...
    def convert_bbox_to_world_coords(self, bbox: list) -> list:
        ...
        print(f"[Orchestrator] 转换完成: 最终世界坐标 ({world_x:.4f}, {world_y:.4f}, {TARGET_Z_HEIGHT})")
        return world_coords
        ...
        for i, task in enumerate(plan):
            ...
            try:
                if task_type == "locate_and_memorize":
                    ...
                    else: raise Exception(f"视觉模型未能找到 '{description}'")
                elif task_type == "move_to_location":
                    name = params['location_name']
                    if not self.robot.move_to_location(name, self.memory): raise Exception(f"机器人移动到 '{name}' 失败。")
                ...
                print(f"!!! 错误：在执行第 {i+1} 步 ({task_type}) 时发生: {e}"); print("!!! 任务计划中止。"); return
...
def main():
    ...
    agent = None; stt_module = None
    try:
        stt_module = SpeechToText()
        agent = TaskOrchestrator(ros_controller_node=ros_controller)
        if not ros_controller.wait_ready(): raise RuntimeError("ROS2/MoveIt 服务初始化失败。")
        if not ros_controller.enable_robot(True): raise RuntimeError("使能机械臂失败。")
        while rclpy.ok():
            command = stt_module.listen(duration=7)
            if command:
                print("\n" + "#"*40); print(f"##  最终识别到的指令: '{command}'  ##"); print("#"*40)
                agent.execute_command(command)
                print("\n任务流程结束。准备接收下一条指令...")
            else:
                print("\n未能识别到有效指令，请重试。")
            continue_prompt = input("按 Enter 键继续下达指令，或输入 'q' 退出: ")
            if continue_prompt.lower() == 'q': break
    except KeyboardInterrupt: print("\n程序被用户中断。")
    except Exception as e:
        error_logger = agent.robot.controller.get_logger() if agent and agent.robot and agent.robot.controller else print
        error_logger(f"Agent执行中发生严重错误: {e}")
    finally:
        if agent: agent.robot.shutdown()
        elif 'ros_controller' in locals() and ros_controller: ros_controller.destroy_node()
