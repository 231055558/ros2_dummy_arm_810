...
class RVizMoveItDemo(Node):
    def __init__(self):
        ...
        # 目标定义改为动态从 target.txt 读取
        self.targets_file = '/home/li/ros2_dummy_arm/target.txt'
    ...
    def _read_target_from_file(self, target_name: str):
        """从targets文件中读取指定名称的目标，返回[rad]*6 或 None。以度格式存储，解析后转弧度。"""
        path = self.targets_file
        if not os.path.exists(path):
            self.get_logger().error(f"❌ 未找到目标文件: {path}")
            return None
        try:
            with open(path, 'r', encoding='utf-8') as f:
                lines = f.readlines()
        except Exception as e:
            self.get_logger().error(f"❌ 读取目标文件失败: {e}")
            return None

        header = f"{target_name.strip()}:".strip()
        i = 0
        while i < len(lines):
            line = lines[i].strip()
            # 跳过空行与注释
            if not line or line.startswith('#'):
                ...
                continue
            if line == header:
                # 收集后续6行 joint1..joint6
                degrees = []
                j = i + 1
                while j < len(lines) and len(degrees) < 6:
                    l = lines[j].strip()
                    # 碰到下一个目标头时中断
                    if l.endswith(':') and not l.startswith('joint'):
                        break
                    # 仅解析 jointX 行
                    if l.lower().startswith('joint'):
                        val = self._parse_degrees_from_line(l)
                        if val is not None:
                            degrees.append(val)
                    j += 1
                if len(degrees) == 6:
                    ...
                    self.get_logger().error(f"❌ 目标{target_name}格式不完整(需要6个joint行)")
                    return None
            i += 1
        self.get_logger().error(f"❌ 在目标文件中未找到: {target_name}")
        return None

    def plan_and_execute_target(self, target_name):
        """规划并自动执行到目标位置(从target.txt动态读取)"""
        # 动态读取（不缓存），支持运行时修改
        target_positions = self._read_target_from_file(target_name)
    ...
    def show_menu(self):
        ...
        print("="*70)
        print("目标与模式:")
        print("  直接输入目标名（如 target1）→ 从target.txt动态读取并执行")
        print("  p. record    - 进入记录点模式：输入英文名立即保存当前位姿，'back'退出")
        print("  o. cruise    - 进入巡航模式：输入记录点名或target.txt目标名立即执行，'back'退出")
        ...
        print("  7. current   - 显示当前状态（输出格式可直接复制到target.txt的joint行）")
    ...
    try:
        ...
        while True:
            ...
            if user_input == '0' or user_input.lower() in ['exit', 'quit']:
                ...
                # 巡航模式
                print("进入巡航模式：输入记录点名或 target.txt 中的目标名，输入 back 退出")
                while True:
